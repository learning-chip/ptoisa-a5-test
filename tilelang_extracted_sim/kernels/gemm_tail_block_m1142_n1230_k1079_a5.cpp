...
AICORE void main_kernel(__gm__ half *A_handle, __gm__ half *B_handle, __gm__ half *C_handle, uint64_t ffts_Addr) {
  ...
  for (int32_t k = 0; k < 17; ++k) {
      ...
      tl::ascend_pto::copy_gm_to_l1<half, half, 1, 1, 1, 128, 64, 1, 1, 1232218, 1079, 1, 128, 64>(A_handle + (((cid / 5) * 138112) + (k * 64)), 0, 0, ((cid <= 39) ? 128 : (1142 - ((cid / 5) * 128))), ((k <= 15) ? 64 : (1079 - (k * 64))));
      tl::ascend_pto::copy_gm_to_l1<half, half, 1, 1, 1, 64, 256, 1, 1, 1327170, 1230, 1, 64, 256>(B_handle + ((k * 78720) + ((cid % 5) * 256)), 16384, 0, ((k <= 15) ? 64 : (1079 - (k * 64))), (((cid % 5) <= 3) ? 256 : (1230 - ((cid % 5) * 256))));
      set_flag(PIPE_MTE2, PIPE_M, EVENT_ID1);
      wait_flag(PIPE_MTE2, PIPE_M, EVENT_ID1);
      pipe_barrier(PIPE_M);
      tl::ascend_pto::gemm_v0<half, float, 128, 256, 64, 128, 256, 64, 64, false, false>(A_L1, B_L1, C_L0, (k == 0));
    }
    set_flag(PIPE_M, PIPE_FIX, EVENT_ID4);
    wait_flag(PIPE_M, PIPE_FIX, EVENT_ID4);
    tl::ascend_pto::copy_l0c_to_gm<half, float, 1, 1, 1, 128, 256, 1, 1, 1404660, 1230, 1, 128, 256>(C_handle + (((cid / 5) * 157440) + ((cid % 5) * 256)), 0, 0, ((cid <= 39) ? 128 : (1142 - ((cid / 5) * 128))), (((cid % 5) <= 3) ? 256 : (1230 - ((cid % 5) * 256))));
#endif
}
...
extern "C" void call(uint8_t *A_handle, uint8_t *B_handle, uint8_t *C_handle, void *stream)
{
    ...
    launch_kernel<<<45, nullptr, stream>>>(A_handle, B_handle, C_handle, fftsAddr);
}
