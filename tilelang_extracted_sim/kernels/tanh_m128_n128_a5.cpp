...
AICORE void main_kernel(__gm__ float *A_handle, __gm__ float *B_handle, uint64_t ffts_Addr) {
  ...
  auto vid = get_subblockid();
    ...
    set_mask_norm();
    set_vector_mask(-1, -1);
    tl::ascend_pto::copy_gm_to_ub<float, float, 1, 1, 1, 32, 64, 1, 1, 16384, 128, 1, 32, 64, pto::PadValue::Zero>(A_handle + ((((cid / 2) * 8192) + (vid * 4096)) + ((cid % 2) * 64)), 0, 0, 32, 64);
    ...
    tl::ascend_pto::copy_ub_to_gm<float, float, 1, 1, 1, 32, 64, 1, 1, 16384, 128, 1, 32, 64>(B_handle + ((((cid / 2) * 8192) + (vid * 4096)) + ((cid % 2) * 64)), 24576, 0, 32, 64);
#endif
}
...
extern "C" void call(uint8_t *A_handle, uint8_t *B_handle, void *stream)
{
    ...
    launch_kernel<<<4, nullptr, stream>>>(A_handle, B_handle, fftsAddr);
}
