...
AICORE void main_kernel(__gm__ int8_t *A_handle, __gm__ int8_t *B_handle, __gm__ float *scale_handle, __gm__ bfloat16_t *C_handle, __gm__ int *workspace_1_handle, uint64_t ffts_Addr) {
  ...
  auto vid = get_subblockid();
    ...
    tl::ascend_pto::copy_gm_to_ub<float, float, 1, 1, 1, 1, 256, 1, 1, 1, 64, 1, 1, 256, pto::PadValue::Zero>(scale_handle + 0, 65536, 0, 1, 64);
...
TCVT(c_out, c_scale, RoundMode::CAST_RINT);
    ...
    tl::ascend_pto::copy_ub_to_gm<bfloat16_t, bfloat16_t, 1, 1, 1, 64, 256, 1, 32768, 4096, 64, 1, 64, 256>(C_handle + ((cid * 4096) + (vid * 4096)), 132096, 0, ((vid == 0) ? 64 : 0), 64);
#endif
}
