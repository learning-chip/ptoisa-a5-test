...
AICORE void main_kernel(__gm__ int8_t *A_handle, __gm__ int8_t *B_handle, __gm__ float *scale_handle, __gm__ half *C_handle, __gm__ int *workspace_1_handle, uint64_t ffts_Addr) {
  ...
  tl::ascend_pto::TileUbDataND<half, 64, 256, 64, 256> c_out;
  TASSIGN(c_out, 132096);
  auto vid = get_subblockid();
#if defined(__DAV_C310_CUBE__)
    tl::ascend_pto::copy_gm_to_l1<int8_t, int8_t, 1, 1, 1, 128, 64, 1, 32768, 4096, 64, 1, 128, 64>(A_handle + (cid * 4096), 0, 0, 64, 64);
    tl::ascend_pto::copy_gm_to_l1<int8_t, int8_t, 1, 1, 1, 64, 256, 1, 32768, 4096, 64, 1, 64, 256>(B_handle + (cid * 4096), 8192, 0, 64, 64);
    ...
    tl::ascend_pto::copy_l0c_to_gm<int, int, 1, 1, 1, 128, 256, 1, 32768, 4096, 64, 1, 128, 256>(workspace_1_handle + (cid * 4096), 0, 0, 64, 64);
    ...
    tl::ascend_pto::copy_gm_to_ub<int, int, 1, 1, 1, 64, 256, 1, 32768, 4096, 64, 1, 64, 256, pto::PadValue::Zero>(workspace_1_handle + ((cid * 4096) + (vid * 4096)), 0, 0, ((vid == 0) ? 64 : 0), 64);
...
TCVT(c_out, c_scale, RoundMode::CAST_RINT);
    ...
    tl::ascend_pto::copy_ub_to_gm<half, half, 1, 1, 1, 64, 256, 1, 32768, 4096, 64, 1, 64, 256>(C_handle + ((cid * 4096) + (vid * 4096)), 132096, 0, ((vid == 0) ? 64 : 0), 64);
#endif
}
...
extern "C" void call(uint8_t *A_handle, uint8_t *B_handle, uint8_t *scale_handle, uint8_t *C_handle, uint8_t *workspace_1_handle, void *stream)
{
    ...
    launch_kernel<<<8, nullptr, stream>>>(A_handle, B_handle, scale_handle, C_handle, workspace_1_handle, fftsAddr);
}
