...
template <int32_t funcKey, typename CType, typename AType, typename BType, int32_t key, uint32_t M, uint32_t K,
          uint32_t N>
bool RunAcc2MatCase(const std::string &dataDir)
{
    size_t aFileSize = M * K * sizeof(AType);
    size_t bFileSize = K * N * sizeof(BType);
    size_t cFileSize = (M - 0) * (N - 0) * sizeof(CType);
...
static const std::vector<CaseSpec> kSmokeCases = {
    {"case_nz2nd_4", RunAcc2MatCase<2, uint32_t, uint16_t, uint16_t, 4, 6, 7, 8>},
};
