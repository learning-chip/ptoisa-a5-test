#!/usr/bin/env python3
"""Self-contained tmul smoke tests (torch_npu + ctypes)."""
...
bind_launch(_LIB, "pto_tmul_float_16x32", 4)
bind_launch(_LIB, "pto_tmul_int32_16x32", 4)


def _make_mul_golden(dtype, dh, dw, s0h, s0w, s1h, s1w, vr, vc):
...
def case_float_16x32():
    ...
    s0 = npu_tensor(in1)
    s1 = npu_tensor(in2)
    _LIB.pto_tmul_float_16x32(data_ptr(out), data_ptr(s0), data_ptr(s1), stream_ptr())
    sync()
...
def case_int32_16x32():
    ...
    s0 = npu_tensor(in1)
    s1 = npu_tensor(in2)
    _LIB.pto_tmul_int32_16x32(data_ptr(out), data_ptr(s0), data_ptr(s1), stream_ptr())
    sync()
