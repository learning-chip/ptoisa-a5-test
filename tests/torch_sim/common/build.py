#!/usr/bin/env python3
"""Bisheng build helper for torch_sim (no runtime_camodel link)."""
...
TORCH_SIM_ROOT = Path(__file__).resolve().parent.parent
PROJECT_ROOT = TORCH_SIM_ROOT.parent.parent
...
def _host_flags(test_dir: Path) -> list[str]:
    return _common_includes(test_dir) + [
        "-std=gnu++17",
        "-O2",
        "-Wno-macro-redefined",
        "-Wno-ignored-attributes",
...
def build_test_lib(
    ...
    test_dir = TORCH_SIM_ROOT / test_name
...
TESTS = {
    "tadd": {"kernels": ["tadd_kernel.cpp"], "arch": "vec"},
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build torch_sim test libraries")
    parser.add_argument("--all", action="store_true", help="Build all test libs")
